...
| D10 | Chat panel dismissal | **Chat closes on Esc or ✕ only — no click-away.** Define panel keeps click-away. | Josh, 2026-07-13: a chat carries conversation state, so a stray click outside must not destroy it; definitions stay disposable. |
| D11 | Distribution signing | **Every release signed with the persistent self-signed identity "TangentBar Release Signing"; installer resets stale TCC entries on signature change; uninstaller (script + Dev menu) clears them.** | 2026-07-13: ad-hoc signing changed the signature every build, so macOS TCC invalidated the Accessibility grant on each update while System Settings kept showing a live toggle that off/on couldn't revive. A stable designated requirement keeps grants across updates. Identity: `~/.config/tangentbar/signing` + GitHub Actions secrets; regenerate only knowingly (`scripts/make-signing-cert.sh`). |
